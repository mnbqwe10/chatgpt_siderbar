(function () {
  const BRIDGE_MESSAGE_SOURCE = "m365-copilot-sidebar-bridge";
  ...
  function injectPromptIntoDocument(promptText, doc = document, win = window) {
    ...
    if (!composer) {
      return { ok: false, error: "M365 Copilot composer not found" };
    }
    if (!setComposerText(composer, promptText, doc, win)) {
      return { ok: false, error: "Unable to populate M365 Copilot composer" };
    }
    if (!submitComposer(composer, doc, win)) {
      return { ok: false, error: "Unable to submit M365 Copilot prompt" };
    }
    return { ok: true };
  }
